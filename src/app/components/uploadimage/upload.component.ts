...
export class UploadComponent implements OnInit, OnDestroy  {
  ...
  upload() {
    if (this.selectedFile.length > 0) {
      const uploadObservables: Observable<any>[] = this.selectedFile.map(file => this.uploadService.upload(file));
  ...
  updateFileInput() {
    ...
    this.selectedFile.forEach(file => dataTransfer.items.add(file));
